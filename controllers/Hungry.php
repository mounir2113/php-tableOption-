...
class Hungry extends CI_Controller
{
	...
	public function __construct()
	{
		...
		$this->load->model("../modules/ordermanage/models/Order_model", "order_model");
	...
	public function get_current_order() 
	{
		...
		$query = $this->db->select('order_id')
			...
			->where("((order_date = '$date' AND order_time > '$time') OR order_date > '$date')")
			->order_by('order_id', 'desc')
			->limit(1)
			->get();
		// var_dump($query);
		...
		if ($query->num_rows() > 0) {
			// Process the data (in this case, just print the ID)
			$single_order_id = $query->row()->order_id;
			// echo "Found order ID: $single_order_id\n";
			...
			// Update the column isPrinted to true for the fetched order ID
			$this->db->set('isPrinted', 1)
				->where('order_id', $single_order_id)
				->update('customer_order');

			// echo "Updated isPrinted to true for order ID: $single_order_id\n";
			$htmlContent = $this->load->view('autoprintInvoiceHtml', $data, true);
			print_r($htmlContent);
			return $htmlContent;
	...
	public function menu()
	{
		...
		if (isset($_GET['custom_id'])) {
			$category = $_GET['custom_id'];
			$productid = NULL;
			$cateid = NULL;
			$dessertCase = $category;
		}
		...
		// add query string in pagination
		// if (isset($getQueryString) && $getQueryString != NULL) { 
		// 	$qu = '?' . http_build_query(['q' => $getQueryString]);
		// } else {
		// 	$qu = "";
		// }

		//$data["links"] = $this->pagination->create_links().$qu;
	...
	public function addtocart()
	{
		...
		$this->session->unset_userdata('special_category');
		$data['title'] = "Article du panier";
		$catid = $this->input->post('catid');
		$especialCategory = $this->input->post('especialCategory');
		$this->session->unset_userdata('especialCategory');

		$this->db->select('CategoryID,Name,price');
		$this->db->from('item_category');
		$this->db->where('item_category.CategoryIsActive', 1);
		$this->db->where('item_category.CategoryID', 1);
		$query = $this->db->get();
		$especial = $query->result_array();
		if ($especialCategory == 1) {
			$this->session->set_userdata('especialCategory', $especial);
		}
		$pid = $this->input->post('pid');
		$sizeid = $this->input->post('sizeid');
		$itemname = $this->input->post('itemname');
		...
		$data_items = array(
			...
			'pid' => $pid,
			'catid' => $catid,
			'especialCategory' => $especialCategory,
			'name' => $itemname,
			'sizeid' => $sizeid,
			'size' => $size,
			'qty' => $qty,
		...
		if ($especialCategory == 1) {
			$cart = $this->cart->contents();
			$data['especialCategory'] = 1;
			$cleanUrl = base_url() . 'menu';
			foreach ($cart as $item) {
				if ($item['catid'] == 1 || $item['catid'] == 2 || $item['catid'] == 3) {
					$matchedCategories[] = $item['catid'];
				}
			}

			$missingCategories = array_diff([1, 2, 3], $matchedCategories);
			if (!empty($missingCategories)) {
				sort($missingCategories);
			}

			if (empty($missingCategories)) {
				$redirectUrl = base_url() . 'cart';
			} else {
				$redirectUrl = $cleanUrl . '?' . $missingCategories[0] . '&' . $especialCategory;
			}
	...
	public function checkout()
	{
		...
		if (!empty($this->cart->contents())) {
			...
			$cart = $this->cart->contents();
			foreach ($cart as $item) {
				$this->session->set_userdata('special_category', $item['especialCategory']);
			}
...
document.getElementById("paytrack").click();
  function payWithPaystack(){
	...
	public function printInvoice($id)
	{
		...
		$htmlContent = $this->load->view('posinvoicedirectprint', $data, true);

		// Specify the printer name (replace with your printer name)
		// $printerName = "RECEIPTITO";
		// echo $this->session->userdata('special_category');
		$printerName = "HP LaserJet Pro MFP M126nw";

		// // Open the printer connection
		// $printer = fopen("smb://$printerName", "w");

		// if ($printer === false) {
		// 	$error = error_get_last();
		// 	echo "Error opening printer: " . $error['message'];
		// } else {
		// 	// Your code for writing to the printer goes here
		// }

		// // Check if the printer connection is successful
		// if ($printer === false) {
		// 	die("Failed to open printer connection.");
		// }
		...
		// 	// Send text to be printed
		// 	fwrite($printer, $html);

		// 	// Cut the paper (if applicable)
		// 	fwrite($printer, "\x1D\x56\x01");
		...
		// echo $printerName = 'HP LaserJet Pro MFP M126nw';
		// echo "<br>";
		// echo $pythonScript = __DIR__ . '\\print.py';
		// echo "<br>";
		// echo "python $pythonScript \"$htmlContent\" \"$printerName\"";

		// $command = "python $pythonScript \"$htmlContent\" \"$printerName\"";
		// $output = [];
		// $returnVar = 0;
